...
Wreads<-otu_table(rMGphylo)[which(tax_table(rMGphylo)[,6]=='Entomoplasma'),]
...
#Define violin plot
p_wolbachia <- ggplot(diversity_df, aes(x=Group, y=Wolbachia)) + geom_violin(aes(fill=Group),scale='width') +labs(x ="Group", y = "Entomoplasma")
...
p_wolbachia<-p_wolbachia+geom_boxplot(aes(fill=Group),width=0.1)+scale_y_continuous(trans='log2')
